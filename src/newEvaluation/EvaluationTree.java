...
public class EvaluationTree
{
			...
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException
	{
		...
		if (tree.get(i).get(0).node.vertextype.equals("0"))
		{ // 表示该节点是定量节点
			switch (tree.get(i).get(0).node.weighting)
			{
			// 表示算数加权平均
			case "01":
				tree.get(i).get(0).node.value = IntegretedMeth.wArithmeticMean(
						weights, values);
				break;
			// 表示几何加权平均
			case "02":
				tree.get(i).get(0).node.value = IntegretedMeth.wGeometricMean(
						weights, values);
				break;
			default:
				JsonStr.result = "01";
		...
		else if (tree.get(i).get(0).node.vertextype.equals("0"))
		{ // 表示该节点是定性节点

			String[] vertexlevel = tree.get(i).get(0).node.vertexlevel
					.split(",");
			...
			tree.get(i).get(0).node.level = (double) IntegretedMeth
					.fuzzyComprehensiveEvaluation(weights, values, vlevel,
							mbsp, conclude).intValue();
			...
			if (tree.get(i).get(0).node.nmlvl.equals("00"))
			{
				...
				var.add(new Variable("x", DataType.DATATYPE_DOUBLE, tree.get(i)
						.get(0).node.level));
				tree.get(i).get(0).node.value = new Double(ExpressionEvaluator
			...
			else
			{
				...
				tree.get(i).get(0).node.value = Meth.exenm(
						tree.get(i).get(0).node.nmlvl, sts,
			...
			InvocationTargetException, NoSuchMethodException, SecurityException
	{
		...
		for (int i = 0; i < nodes.size(); i++)
		{
			if (nodes.get(i).nclass.equals("2"))
			{
				...
				{
					Sts sts = new IdxSts(nodes.get(i).id);
					nodes.get(i).value = Meth.exenm(nodes.get(i).nmidix, sts,
							temp);
					System.out.println("####################################");
				}

				// nodes.get(i).value = rdb.getValueOfIndex(evaluateId,
				// wpindexid, expid);
